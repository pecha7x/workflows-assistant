  ...
  class Message < TelegramControllerTest
    class Authentication < Message
      test 'post message without token should be forbidden' do
        ...
        assert_response 403
        assert_equal response.parsed_body['errors']['message'], 'Access denied.'
      end
      ...
      test 'post message with valid token should return success response' do
        post telegram_message_path, headers: { 'X-Telegram-Bot-Api-Secret-Token': Rails.application.credentials.telegram.webhook_token }
        assert_response 200
      end
    end  
  end
